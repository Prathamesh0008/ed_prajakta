...
export default function ProductDetailsPage() {
  ...
  const isInStock = productData?.inStock !== false;
...
const getTierPrice = () => {
  ...
  const tier = productData.pricing.find(
    (range) => quantity >= range.min && quantity <= range.max
  );

  return tier ? tier.price : productData.pricing[0].price;
};
...
<div className="text-sm text-gray-500">
  ...
  className="w-20 px-2 py-2 border-x border-gray-300 text-center text-gray-900 outline-none"
/>
  ...
  <div className="flex justify-between text-sm text-gray-600">
    ...
    <span className="font-medium text-[#8B0035]">
      €{getTierPrice().toFixed(2)}

    </span>
  </div>

  <div className="flex justify-between text-base font-semibold mt-2c text-black">
    <span>Total:</span>
    <span className="text-[#8B0035]">
€{(getTierPrice() * quantity).toFixed(2)}
  ...
  className={`flex-1 py-3 font-semibold rounded-lg transition-all duration-300 flex items-center justify-center gap-2 ${
    isInStock
      ? "bg-gradient-to-r from-[#8B0035] to-[#6b0028] text-white hover:shadow-lg"
